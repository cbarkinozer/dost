...
<div class="p-2 space-y-2">
	<button
		...
		class="w-full flex items-center justify-between p-3 rounded-lg text-left text-sm font-medium text-gray-200 hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
	>
		<span>New Chat</span>
		<PencilIcon />
	</button>
	<div class="relative">
		...
		<input
			...
			class="w-full pl-8 pr-3 py-2 bg-[#0d1117] border border-gray-700 rounded-lg text-sm text-gray-200 focus:ring-blue-500 focus:border-blue-500"
		/>
	...
	<ul class="space-y-1">
		...
		{#each $filteredConversations as conv (conv.id)}
			<li class="group relative">
				...
				>
                    {#if conv.pinned}
                        <div class="text-amber-400"><PinIcon /></div>
                    {/if}
					<span class="flex-1 truncate">{conv.title}</span>
				</a>
                <div class="absolute right-2 top-1/2 -translate-y-1/2 flex items-center opacity-0 group-hover:opacity-100 bg-[#161b22] rounded-full">
                    <button on:click={(e) => handlePin(e, conv.id)} title="Pin conversation" class="p-1.5 text-gray-400 hover:text-amber-400">
                        <PinIcon />
                    </button>
                    <button on:click={(e) => handleDelete(e, conv.id)} title="Delete conversation" class="p-1.5 text-gray-400 hover:text-red-400">
                        <TrashIcon />
                    </button>
                </div>
			</li>
...
<div class="mt-auto p-2 border-t border-gray-700 space-y-1">
	<a href="/documents" class="flex items-center gap-3 p-2 rounded-lg text-gray-200 hover:bg-gray-700">
		<DocumentTextIcon />
	...
	<a href="/settings" class="flex items-center gap-3 p-2 rounded-lg text-gray-200 hover:bg-gray-700">
		<div class="w-8 h-8 rounded-full bg-gray-600 flex items-center justify-center text-white font-bold">U</div>
